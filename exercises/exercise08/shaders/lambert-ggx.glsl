...
vec3 GetAlbedo(SurfaceData data)
{
	...
	vec3 albedo = mix(data.albedo, vec3(0.0f), data.metalness);
...
vec3 GetReflectance(SurfaceData data)
{
	...
	vec3 reflectance = mix(vec3(0.04f), data.albedo, data.metalness);		// We use a fixed value for dielectric, with a typical value for these materials (4%)
...
vec3 ComputeSpecularIndirectLighting(SurfaceData data, vec3 viewDir)
{
	...
	vec3 specularLighting = SampleEnvironment(reflectionVector, pow(data.roughness, 0.25f));

	// (todo) 08.6: Add a geometry term to the indirect specular
	float geometry = GeometrySmith(data.normal, reflectionVector, viewDir, data.roughness);
...
vec3 ComputeSpecularLighting(SurfaceData data, vec3 lightDir, vec3 viewDir)
{
    ...
    float n_v = ClampedDot(data.normal, viewDir);
    float n_l = ClampedDot(data.normal, lightDir);

    float denominator = 4.0f * n_v * n_l + 0.00001f;
